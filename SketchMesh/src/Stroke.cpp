...
void Stroke::strokeAddSegment(int mouse_x, int mouse_y) {
	...
	if (!empty2D()) {
		...
		if (timePast < 1000000) { //Don't add another segment before x nanoseconds
			return;
		}
	}
	...
	pt = igl::unproject(Eigen::Vector3f(x, y, 0.05*dep), modelview, viewer.core.proj, viewer.core.viewport).transpose().cast<double>();
